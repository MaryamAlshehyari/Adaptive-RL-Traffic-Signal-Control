...
if __name__ == "__main__":
    ...
    prs.add_argument(
        ...
        dest="route",
        type=str,
        default="/sumo-rl/sumo_rl/nets/RESCO/cologne1/cologne1.rou.xml",
    ...
    out_csv = f"/sumo-rl/outputs/ql/ql_2way-single-intersection_cologne1/{experiment_time}_alpha{args.alpha}_gamma{args.gamma}_eps{args.epsilon}_decay{args.decay}_reward{args.reward}"
    env = SumoEnvironment(
    ...
    for run in range(1, args.runs + 1):
        ...
        q_table_dir = os.path.join(out_csv, "q_tables")
        os.makedirs(q_table_dir, exist_ok=True)

        # Save Q-table for each agent
        for ts_id, agent in ql_agents.items():
            q_table_path = os.path.join(q_table_dir, f"{ts_id}_q_table.pkl")
